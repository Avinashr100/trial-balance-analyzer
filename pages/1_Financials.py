def generate_statement(df, month_col, section_order):
    ...
    curr = df_curr.groupby(["Account Category", "Account Name", "Account Type"]).agg({"Debit": "sum", "Credit": "sum"}).reset_index()
    prev = df_prev.groupby(["Account Category", "Account Name", "Account Type"]).agg({"Debit": "sum", "Credit": "sum"}).reset_index()
